...
tape( 'main export is an object', function test( t ) {
	...
	t.strictEqual( typeof zswap, 'object', 'main export is an object' );
	t.end();
});

tape( 'the `ndarray` method has an arity of 7', function test( t ) {
	t.strictEqual( zswap.ndarray.length, 7, 'returns expected value' );
	t.end();
});

tape( 'the `ndarray` method swaps elements in `x` and `y`', function test( t ) {
	var viewX;
	var viewY;
	var xe;
	var ye;
...
tape( 'the `ndarray` method returns a reference to the destination array', function test( t ) {
	...
	out = zswap.ndarray( x.length, x, 1, 0, y, 1, 0 );
...
tape( 'if provided an `N` parameter less than or equal to `0`, the `ndarray` method returns both vectors unchanged', function test( t ) {
	...
	zswap.ndarray( -1, x, 1, 0, y, 1, 0 );
	t.deepEqual( viewX, xe, 'returns expected value' );
	t.deepEqual( viewY, ye, 'returns expected value' );

	zswap.ndarray( 0, x, 1, 0, y, 1, 0 );
	t.deepEqual( viewX, xe, 'returns expected value' );
	t.deepEqual( viewY, ye, 'returns expected value' );
...
tape( 'the `ndarray` method supports negative strides', function test( t ) {
	...
	zswap.ndarray( N, x, -2, x.length-2, y, -1, y.length-2 );
...
tape( 'the `ndarray` method supports complex access patterns', function test( t ) {
	...
	zswap.ndarray( N, x, 2, 1, y, -1, y.length-1 );
...
tape( 'if both strides are equal to `1`, the `ndarray` method efficiently swaps elements in `x` and `y`', function test( t ) {
	var viewX;
	var viewY;
	var xe;
	var ye;
